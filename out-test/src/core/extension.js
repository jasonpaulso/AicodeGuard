...
async function activate(context) {
    ...
    const commandDisposables = [
        ...
        vscode.commands.registerCommand('ai-code-guard.quickConfig', async () => {
            ...
            if (selected) {
                const configManager = require('../managers/ConfigManager').ConfigManager.getInstance(monitor.outputChannel);
                await configManager.setMonitoringMode(selected.value);
        ...
        vscode.commands.registerCommand('ai-code-guard.enforceQuality', () => {
            vscode.window.showInformationMessage('🛡️ Quality Enforcement: Ensuring production-ready implementations', 'Continue', 'Cancel').then(selection => {
        ...
        vscode.commands.registerCommand('ai-code-guard.enableIntervention', () => {
            ...
            vscode.window.showInformationMessage('🛡️ Implementation enforcement enabled. AI Code Guard will actively ensure complete implementations.');
        }),
        // Disable intervention command
        vscode.commands.registerCommand('ai-code-guard.disableIntervention', () => {
            monitor.disableIntervention();
            vscode.window.showInformationMessage('👁️ Switched to monitoring mode. AI Code Guard will track but not intervene in quality issues.');
        }),
        ...
        vscode.commands.registerCommand('ai-code-guard.analyzeSelection', () => {
            ...
            if (editor && editor.selection) {
                ...
                if (selectedText) {
                    vscode.window.showInformationMessage(`Analyzing ${selectedText.length} characters...`, 'Show Analysis');
                }
            }
        }),
